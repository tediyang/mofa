$((function() {
  /**
   * Fetches the beneficiary associated with a given benefactor id.
   * @param {number} benefactorId The id of the benefactor.
   * @returns {Object} The beneficiary object associated with the given benefactor id.
   */
  function fetchBeneficiary(benefactorId) {
    ...
    return allBeneficiaries.data.filter(beneficiary => beneficiary.id === parseInt(benefactorId))[0];
  }

  /**
   * Display the details of a benefactor with the given ID.
   * @param {string} benefactorId - The ID of the benefactor to display.
   */
  function displayBeneficiary(benefactorId) {
    if (benefactorId) {
      const benefactorData = fetchBeneficiary(benefactorId);
      if (benefactorData) {
        $('.section .img-box__img img').attr({
          src: benefactorData.image,
          alt: benefactorData.name + " image"
        });
        $('.heading__pre-title').text(benefactorData.illness);
        $('.heading__title span').text(benefactorData.name);
        $('.section .description').text(benefactorData.history);
        $('.text-center').html(`<a href="#" class="align-self-center button button--${benefactorData.type.toLowerCase()} mx-auto my-3">Donate</a>`) // dynamic button based on content
        // Populate other details...
      } else {
        window.location.href = "404.html"; // throw 404
      }
    } else {
      window.location.href = "404.html"; // throw 404
    }
  ...
  if ($('#content-start').length) { // Check if the element exists
    $('html, body').animate({
      scrollTop: $('#content-start').offset().top
    }, 200); // Adjust scroll speed (500ms) as needed
  }

  /**
   * Retrieves a URL parameter value.
   *
  ...
  const benefactorId = getUrlParameter('id');

  displayBeneficiary(benefactorId);
}));
